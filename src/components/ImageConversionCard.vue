...
const algorithm = ref<'Basic' | 'Dither' | 'Atkinson' | 'Bayer' | 'SierraLite'>(
    props.initialOptions?.algorithm ?? 'Basic',
);
...
const paperThickness = ref<'none' | 'light' | 'medium' | 'heavy' | 'dedicated'>(
    props.initialOptions?.paperThickness ?? 'none',
);
const preprocessFilter = ref<'none' | 'portrait' | 'pet' | 'lineplus' | 'auto' | 'draft'>(
    props.initialOptions?.preprocessFilter ?? 'none',
);
const filterOrder = ref<'before-resize' | 'after-resize'>(
    props.initialOptions?.filterOrder ?? 'before-resize',
);
const imageSmoothingEnabled = ref(props.initialOptions?.imageSmoothingEnabled ?? true);
const imageSmoothingQuality = ref<'low' | 'medium' | 'high'>(
    props.initialOptions?.imageSmoothingQuality ?? 'high',
);
const resizeAlgorithm = ref<'canvas' | 'nearest' | 'linear' | 'cubic' | 'area' | 'lanczos4'>(
    props.initialOptions?.resizeAlgorithm ?? 'canvas',
);
const sharpenBeforeResize = ref<'none' | 'light' | 'medium' | 'strong'>(
    props.initialOptions?.sharpenBeforeResize ?? 'none',
);
const sharpenAfterResize = ref<'none' | 'light' | 'medium' | 'strong'>(
    props.initialOptions?.sharpenAfterResize ?? 'none',
);
...
const imageConversionOptions = computed(
...
const heightInCm = computed(() => {
    if (!props.originalWidth || !props.originalHeight || !props.pixelPerLine || !props.cmPerLine)
        return null;
        ...
        <CardContent>
            <div class="mb-4">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <Loader2
                        v-if="props.isProcessing && preprocessFilter !== 'none'"
                        :size="16"
            ...
            <div class="mb-4" v-if="preprocessFilter !== 'none'">
                ...
                <ToggleGroup type="single" v-model="filterOrder" id="filter-order">
                    <ToggleGroupItem
                        v-for="option in filterOrderOptions"
                        :key="option"
                        :value="option"
                        @click="emit('image-conversion-options-change', imageConversionOptions)"
                    >
                ...
                <div class="space-y-2">
                    <div class="flex items-center space-x-2">
                        <Switch
                            ...
                            :disabled="props.isProcessing"
                            @update:model-value="
                                emit('image-conversion-options-change', imageConversionOptions)
                            "
                        />
                        <Label for="auto-levels" class="cursor-pointer"> Auto Levels </Label>
                        <Loader2
                            v-if="props.isProcessing && autoLevels"
                            :size="16"
                            class="animate-spin text-blue-500"
                        />
                    </div>
                    <div class="flex items-center space-x-2">
                        <Switch
                            ...
                            "
                        />
                        <Label for="auto-contrast" class="cursor-pointer"> Auto Contrast </Label>
                        <Loader2
                            v-if="props.isProcessing && autoContrast"
                            :size="16"
                            class="animate-spin text-blue-500"
                        />
                    </div>
                    <div class="flex items-center space-x-2">
                        <Switch
                            ...
                            "
                        />
                        <Label for="auto-exposure" class="cursor-pointer"> Auto Exposure </Label>
                        <Loader2
                            v-if="props.isProcessing && autoExposure"
                            :size="16"
                            class="animate-spin text-blue-500"
                        />
                    </div>
            ...
            <div class="mb-4">
                ...
                <ToggleGroup type="single" v-model="paperThickness" id="paper-thickness">
                    <ToggleGroupItem
                        v-for="option in paperThicknessOptions"
                        :key="option"
                        :value="option"
                        @click="emit('image-conversion-options-change', imageConversionOptions)"
                    >
            ...
            <div class="mb-4">
                <Label class="block mb-2 font-medium" for="resize-algorithm"
                    >Resize Algorithm</Label
                >
                <ToggleGroup type="single" v-model="resizeAlgorithm" id="resize-algorithm">
                    <ToggleGroupItem
                        v-for="option in resizeAlgorithmOptions"
                        :key="option"
                        :value="option"
                        @click="emit('image-conversion-options-change', imageConversionOptions)"
            ...
            <div class="mb-4">
                <div class="flex items-center gap-2 mb-2">
                    <Label class="font-medium" for="sharpen-before-resize"
                        >Sharpen Before Resize</Label
                    >
                    <Badge variant="secondary" class="text-xs">Preserves details</Badge>
                    <Loader2
                        v-if="props.isProcessing && sharpenBeforeResize !== 'none'"
                        :size="16"
                        class="animate-spin text-blue-500"
                    />
                </div>
                <ToggleGroup
                    type="single"
                    v-model="sharpenBeforeResize"
                    id="sharpen-before-resize"
                    :disabled="props.isProcessing"
                >
                    <ToggleGroupItem
                        v-for="option in sharpenOptions"
                        :key="option"
                        :value="option"
                        @click="emit('image-conversion-options-change', imageConversionOptions)"
                    >
            ...
            <div class="mb-4">
                <div class="flex items-center gap-2 mb-2">
                    <Label class="font-medium" for="sharpen-after-resize"
                        >Sharpen After Resize</Label
                    >
                    <Badge variant="secondary" class="text-xs">For thermal printing</Badge>
                    <Loader2
                        v-if="props.isProcessing && sharpenAfterResize !== 'none'"
                        :size="16"
                        class="animate-spin text-blue-500"
                    />
                </div>
                <ToggleGroup
                    type="single"
                    v-model="sharpenAfterResize"
                    id="sharpen-after-resize"
                    :disabled="props.isProcessing"
                >
                    <ToggleGroupItem
                        v-for="option in sharpenOptions"
            ...
            <div class="mb-4" v-if="resizeAlgorithm === 'canvas'">
                ...
                <div class="flex items-center space-x-2 mb-2">
                    <Switch
                        id="image-smoothing"
                        v-model="imageSmoothingEnabled"
                        @update:model-value="
                            emit('image-conversion-options-change', imageConversionOptions)
                        "
                    />
                    <Label for="image-smoothing"> Enable Image Smoothing </Label>
                </div>
                <div v-if="imageSmoothingEnabled">
                    <Label class="block mb-2 text-sm text-muted-foreground" for="smoothing-quality"
                        >Smoothing Quality</Label
                    >
                    <ToggleGroup
                        type="single"
                        v-model="imageSmoothingQuality"
                        id="smoothing-quality"
                    >
                        <ToggleGroupItem
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="icon"
                            class="h-6 w-6"
                            @click="adjustContrast(-0.01)"
                            title="Decrease contrast by 0.01"
                        >
                            <ChevronLeft :size="14" />
                        </Button>
                        <Badge
                            variant="outline"
                            class="text-xs font-semibold min-w-[3rem] text-center"
                            >{{ contrast.toFixed(2) }}</Badge
                        >
                        <Button
                            variant="outline"
                            size="icon"
                            class="h-6 w-6"
                            @click="adjustContrast(0.01)"
                            title="Increase contrast by 0.01"
                        >
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="icon"
                            class="h-6 w-6"
                            @click="adjustExposure(-0.01)"
                            title="Decrease exposure by 0.01"
                        >
                            <ChevronLeft :size="14" />
                        </Button>
                        <Badge
                        ...
                        <Button
                            ...
                            class="h-6 w-6"
                            @click="adjustExposure(0.01)"
                            title="Increase exposure by 0.01"
                        >
                            <ChevronRight :size="14" />
                        </Button>
                    </div>
            ...
            <div class="mb-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="icon"
                            class="h-6 w-6"
                            @click="adjustWidthPercentage(-1)"
                            title="Decrease width by 1%"
                        >
                            <ChevronLeft :size="14" />
                        </Button>
                        <Badge
                ...
                <input
                    ...
                    min="1"
                    max="100"
                    v-model.number="widthPercentage"
                    class="w-full"
                    @input="emitChanges"
                />
            </div>
            <div class="mb-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="icon"
                            class="h-6 w-6"
                            @click="adjustHeightPercentage(-1)"
                            title="Decrease height by 1%"
                        >
                            <ChevronLeft :size="14" />
                        </Button>
                        <Badge
                ...
                <input
                    ...
                    min="1"
                    max="100"
                    v-model.number="heightPercentage"
                    class="w-full"
                    @input="emitChanges"
                />
            </div>
